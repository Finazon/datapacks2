...
execute as @e[type=zombie, tag=lvl1zomb] at @s if score @s health <= @s death run summon item ~ ~1 ~ {Item:{id:"minecraft:poisonous_potato",Count:1b,tag:{display:{Name:'{"text":"Rotten Potato","italic":false,"color": "dark_green"}'},Lore:['{"color":"gold","text":"A rotting potato held by a zombie"}','{"color":"gold","text":"Heals 2 health"}']}}}
